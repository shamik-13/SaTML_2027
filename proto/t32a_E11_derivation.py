...
Runs before t32_B1_addis_state.py is patched.  Same contract as t34a/t35a/t36a: every
closed form the exported arrays must satisfy is derived here and checked, so a
disagreement between the export and this file is a bug rather than a finding
(docs/01_HANDOFF_PHASE4.md section 6).

No data files are read.  Runtime ~10 s.

======================================================================================
WHY AN "EXPORT" NEEDS A DERIVATION AT ALL
======================================================================================
E11 is scoped in docs/02_WORKPLAN_PHASE4.md as a 0.1-day patch that writes per-episode
arrays so Figure 4B is plottable from JSON alone.  That much is bookkeeping.  But the
figure's whole content is the COINCIDENCE that section 4.33 reports as two integers:

    detected episodes where ADDIS's level cap binds (alpha_t = lam) : 101 of 147
    detected episodes whose cheapest suppressing pad lands in (lam, tau] : 101 of 147

Plotting 147 points that happen to agree invites exactly the wrong reading -- that the
equality is an empirical accident of this window, which a second dataset might break.
It is not.  Below, D5 proves the equality is FORCED: on any stream whose evidence ceiling
exceeds 1/(tau-lam) = 4, cap-saturation IMPLIES landing, and the converse fails only for
levels within one conformal floor unit of lam.  The figure therefore shows a theorem with
its boundary case, not a correlation, and the export must carry the columns that let a
reader check that.

======================================================================================
NOTATION
======================================================================================
    M            evidence ceiling (|C|+1)/k, k = 1; per-flow e in {0, M} (section 2.2)
    n            episode size in flows (h_stream.build_episodes 'nsz')
    m            number of FIRING flows in the episode, m = sum_e / M ('m_fire')
    Ev           = M*m/n            arithmetic-mean merged evidence (section 2.3)
    P            = min(1, 1/Ev) = min(1, n/(M*m))     Markov p-value (h6_procs.Ctx)
    alpha_t      the level ADDIS offers at that step (h6_procs.run_addis 'levels')
    lam, tau     ADDIS discard/candidate thresholds, 0.25 and 0.5 throughout the record
    r            number of appended non-firing pad flows
======================================================================================
"""
...
# =======================================================================================
# The two primitives, written once.  The patched t32 must agree with these EXACTLY.
# =======================================================================================
def pval(n, m, M):
...
def n_need(alpha_t, m, M):
    ...
    Rejection is P <= alpha_t with alpha_t <= lam < 1, so suppression needs
    n/(M*m) > alpha_t, i.e. n > alpha_t*M*m.  The smallest such integer is
    floor(alpha_t*M*m) + 1 -- correct whether or not alpha_t*M*m is itself an integer,
    which floor(x)+1 handles and ceil(x) does NOT (ceil(3.0) = 3 is not > 3).
    """
    return math.floor(alpha_t * M * m) + 1.0
...
for _ in range(20000):
    ...
    N = n_need(a, m, M)
    if not pval(N, m, M) > a:            # N* must suppress
        bad_hi += 1
    if N - 1 >= 1 and pval(N - 1, m, M) > a:   # N*-1 must NOT suppress
        bad_lo += 1
check("D2a  N* suppresses            (20000 random cases)", bad_hi, 0, 0)
check("D2a  N*-1 does not suppress   (20000 random cases)", bad_lo, 0, 0)

# The integer boundary case that separates floor(x)+1 from ceil(x).
...
for _ in range(50000):
    ...
    hi = math.floor(a * M * m)
    if hi < 1:
        continue                                  # no rejectable size exists at this level
    n = float(rng.integers(1, hi + 1))            # any n that ADDIS would have rejected
    if not pval(n, m, M) <= a:
        continue
    if n_need(a, m, M) - n < 1:
        bad_r += 1
...
for _ in range(50000):
    ...
    if not (a < p_after <= a + 1.0 / X + 1e-15):
        bad_b += 1
    # (p+ - alpha)*X = N* - alpha*X = floor(alpha*X) + 1 - alpha*X = 1 - frac(alpha*X),
    # so the excess is EXACTLY one minus the fractional part -- not merely bounded by 1.
    # Computed as (N/X - a)*X this is a DIFFERENCE OF NEAR-EQUAL FLOATS (both ~alpha)
    # rescaled by X ~ 1e10, so it carries an absolute error of order eps*alpha*X ~ 1e-6.
    # The identity is exact in exact arithmetic (checked with Fraction below); here the
    # residual is checked against that float64 bound rather than against zero, because a
    # tolerance of 0 would fail on rounding and a loose one would have no power.
    ex = (p_after - a) * X
    # The cancellation is between p_after*X = N and a*X, so the error scales with the
    # LARGER of the two, not with a*X.  A bound of 4*eps*a*X is false whenever a*X << N --
    # at a = 1e-9, X = 10 it claims 8.9e-24 against an actual residual of 1.1e-16.
...
# The bound must also hold in the regime that broke the earlier version of it.
...
# The identity itself, in EXACT arithmetic, so no rounding can hide an algebra error.
from fractions import Fraction as Fr
bad_exact = 0
for _ in range(2000):
    M = int(rng.integers(4, 10**7)); m = int(rng.integers(1, 5000))
    a = Fr(int(rng.integers(1, 10**9)), 4 * 10**9)          # a rational alpha <= lam
    X = Fr(M * m)
...
# The supremum is ATTAINED, at alpha*M*m exactly integral -- checked directly rather than
# hoped for from random draws, which approach 1 but never reach it.
...
""")
# (a) verified exhaustively over the whole feasible (M, m) grid, not sampled.
bad_a = 0
...
# The sufficient bound X >= 1/(tau-lam) is not sharp.  Scan every integer X to find where
# SAT => LAND ACTUALLY fails, rather than asserting the sufficient bound is tight.
...
# (b) A blind random search finds NO unsaturated landers at all (the window is ~5e-7 wide
# out of 0.25), so it has no power to test D5b.  Draw alpha DELIBERATELY inside and just
# outside the claimed boundary window instead, and check both directions.
bad_b2 = 0; found_near = 0; missed_far = 0
for _ in range(200000):
    M = float(rng.integers(10**4, 10**7)); m = float(rng.integers(1, 5000))
    X = M * m
    inside = rng.random() < 0.5
    # inside:  lam - alpha in (0, 1/X)      outside: lam - alpha in [1/X, 10/X)
    ...
    if landed:
        found_near += 1
        if LAM - a >= 1.0 / X:
            bad_b2 += 1                 # a lander OUTSIDE the window: would refute D5b
    elif not inside:
        missed_far += 1                 # correctly no landing outside the window
...
# The concrete unsaturated lander, so D7a's proviso is visibly non-vacuous.
...
# (c) the counting consequence, on synthetic streams.
viol = 0
for _ in range(3000):
...
""")
# I1-I8 exercised end to end on a synthetic detected set built the way t32 builds one.
T = 4000
M = 1_813_114.0
...
pad = N - nsz
p_after = N / (M * m_fire)
# EXACT, not np.isclose: run_addis sets alpha_t = min(lam, .), so the cap binds iff
# alpha_t == lam.  np.isclose's default rtol gives 2.5e-6 at lam = 0.25, five times wider
# than the conformal floor 1/M = 5.5e-7 that separates the two regimes, and it will label
# an unsaturated non-lander (alpha_t = 0.249998, M*m = 1e6, p+ = 0.249999) as saturated.
sat = alpha_t >= LAM

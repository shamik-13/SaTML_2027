...
F3 rests on (SrcIP, DstIP, time-bucket) alone.  This adds:

  src-dst     (SrcIP, DstIP, bucket)          the section 4.9 definition, as control
  src         (SrcIP, bucket)                 host-only, attacker side
  dst         (DstIP, bucket)                 host-only, victim side
  subnet24    (Src/24, Dst/24, bucket)        /24 subnet pairs
  src-dport   (SrcIP, DstPort, bucket)        service-based

and finishes the two runs that timed out earlier: the 86,400 s bucket, and host-pair-only
(no time field at all, so an episode is a source-destination pair over the whole window).

The question F3 answers is whether feasibility is a property of the grouping or of the
horizon.  Grouping changes T, so it changes the required calibration set |C| >= kT/w0 - 1
directly.  It also changes the flow-vs-episode gap of F4, because coarser grouping makes
episodes larger and rarer.

k = 1, q = 0.05, w0 = 0.025, position 0.85 and 0.62 (the best and one of the worst tail-reach
positions), two detector seeds each.
"""
...
t0 = time.time()
W0 = 0.025; A = 0.05; K = 1
# The record's H4 result is the two-position run and stays the default so out/t26_H4.json
# reproduces unchanged.  E6 (t39_E6_transfer.py) needs the same grid across ALL FIVE
# positions in order to select a grouping on one window and evaluate it on another; that
# run is requested with --five and writes a SEPARATE file, so neither result can overwrite
# the other.  docs/02_WORKPLAN_PHASE4.md E6 calls this re-run a prerequisite.
...
SEEDS = [0, 1]
BUCKETS = [300, 1800, 3600, 7200, 21600, 86400, None]   # None = host-pair only
...
for pos in POS:
    ...
    for seed in SEEDS:
        ...
        tail_reach = float((e_te[y_te == 1] > 0).mean())   # attack flows at the ceiling
        for fam, keys in fams.items():
            for b in BUCKETS:
                ep = hs.build_episodes(e_te, y_te, ts_w, bucket_s=b, keys=keys)
                T = ep["T"]
                # CEIL is already (|C|+1)/k; dividing by k again double-counts it
                margin = CEIL * W0 / T - 1.0
                req_C = K * T / W0 - 1.0
                ctx = Ctx(ep["Ev"], ep["ismal"], CEIL, alpha=A, w0=W0)
                g1p, g0p = make_gamma("poly", T); g1u, _ = make_gamma("uniform", T)
                # F4's flow-level number is the fraction of malicious FLOWS that sit inside
                # an ALERTED episode -- not the fraction of attack flows reaching the ceiling.
...
# bucket_s=None means "no time field at all".  For src-dst that IS the host-pair-only
# run the handoff asked for; for the other families it is per-source / per-subnet /
# per-service aggregation over the whole window, which is NOT a host pair.

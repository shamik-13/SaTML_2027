...
sequence chosen after looking at the window they are evaluated on?

Reviewer objection (docs/02_WORKPLAN_PHASE4.md E6).  Accept when the main qualitative
findings are shown not to require test-window tuning, or the ones that do are named.

Every design rule below is derived and numerically verified in t39a_E6_derivation.py.  A
disagreement between the two is a bug, not a finding.  The derivation changed the design in
four ways, and each is marked [D..] where it bites:

  D2  a flat grid makes every selection rule look perfect, so normalised regret is reported
      as UNDEFINED rather than 0 when the grid is flat, and the raw spread is printed beside
      every regret.
  D3  EPISODE RECALL IS NOT COMPARABLE ACROSS GROUPING FAMILIES -- the family sets the
      denominator, so selecting on recall selects coarseness.  Selection is on FLOW
      COVERAGE; recall is reported alongside so the gap is visible.
  D4  the selection-window value is a maximum over the grid and overstates.  Both the
      selection-window and evaluation-window values are reported, and the drop between them
      is the winner's curse.
  D5  ties are the common case; the tie-break order is declared in the source below, before
      any file is read, and the number of tied winners is reported.
  D6  the feasibility gate is a function of |C|, k and T only, so it is not a tuned
      parameter and transfers exactly.  Whether the feasible SET moves is reported
      separately.

Reads out/t26_H4_5pos.json (the five-position grouping grid; produced by
`t26_H4_grouping.py --five`) and out/t25_H5.json (the cap sweep, already five-position).
Reads no flow data and fits no detector: runtime ~2 s.

Outputs out/t39_E6.json.
"""
...
# ----------------------------------------------------------------------------------
# [D5a] THE TIE-BREAK ORDER, DECLARED BEFORE ANY FILE IS OPENED.
# Lower priority number wins a tie.  The order is the record's own listing order in
# section 4.9 and docs/02_WORKPLAN_PHASE4.md, NOT a ranking by any measured quantity, and
# NOT the order the JSON happens to store rows in.
# ----------------------------------------------------------------------------------
...
FROZEN_GROUPING = ("src-dst", 7200)     # the record's headline (section 4.9)
FROZEN_CAP = "p99"                      # the record's headline (section 4.12)
OBJ = "flow_cov_addis"                  # [D3b] ADDIS + gamma prop j^-1.6: no horizon oracle
OBJ_ORACLE = "flow_cov_elond"           # e-LOND + horizon-uniform gamma: [ORACLE] control
...
# =======================================================================================
# The selection primitives.  Module level so t39b_E6_selftest.py can import and mutate the
# SHIPPED code rather than a copy of it.
# =======================================================================================
def select(values, order):
    """argmax with a PRE-DECLARED deterministic tie-break.  Returns (index, n_tied).

    `order` is the tie-break priority, LOWER WINS, fixed in this file's source before any
    data is read.  n_tied is returned so a selection among k tied configurations is
    reported as the coin flip it is rather than as an optimisation.                [D5a]
    """
...
def normalised_regret(values_on_eval, achieved):
    ...
    spread = hi - lo
    # A tolerance, not `<= 0`: a grid flat to 1e-15 divides by float noise and can report
    # rho = 900 for an achieved value a picometre below the maximum.
...
def transfer_table(cfgs, order, value, sel_windows, eval_window, frozen_idx, feas=None):
    ...
    value[w][c] is the objective of config c on window w.  Selection averages over the
    selection windows (one window for E6a, four for E6b).

    feas[w][c], if given, is the feasibility mask.  Selection is restricted to configs
    feasible on the SELECTION windows and the oracle to configs feasible on the EVALUATION
    window: an "oracle" that picks a configuration the feasibility theorem forbids is not
    an oracle, it is an unattainable upper bound, and using it inflates every regret.
    Feasibility uses only |C|, k and T (F5/D6a): no detector scores and no
    EVALUATION-window labels.  It is not label-free in general -- |C| is the count of
    flows LABELLED benign in the calibration window -- but it uses nothing from the window
    it is evaluated on, which is what E6 is testing.
    """
    n = len(cfgs)
    # [D4-audit] average only over configs present on EVERY selection window.  np.nanmean
    # over different supports lets a config with [NaN, 1.0] beat one with [0.7, 0.7].
    ...
    # A flat SELECTION window makes the "selection" a pure tie-break: the row is still an
    # operational outcome (something has to be deployed) but it is not evidence about
    # transferability, and averaging it into a worst-transfer summary is how a degenerate
    # window gets reported as a transfer failure.  Flagged, not dropped.
...
def run_axis(name, cfgs, order, value, alt, frozen_cfg, obj_label=OBJ,
    ...
    `alt` is a SECOND objective, used only to show what a different choice of objective
    would have selected [D3a].  It must be a genuinely different quantity: passing `value`
    itself produces a block that reports zero disagreements by construction and reads like
    a result.
    """
    if alt is value or (isinstance(alt, np.ndarray) and alt is value):
        raise ValueError("alt must differ from value -- see D3a")
    # The frozen configuration is passed in, not inferred from the tie-break priority.
    # Deriving it as "the config with priority 0" is correct only while the record's choice
    # happens to head the declared order, and would silently report a different config's
    # numbers under the frozen label if that order were ever edited.
    ...
    # [D3a] the same selection run on the ALTERNATIVE objective, to show what a different
    # choice of objective would have picked.
    rec_pick = []
    for w, p in enumerate(POS):
...
def grid_feasible(rows, seed):
    ...
    (F5 / D6a).  |C| is the number of CALIBRATION-window flows labelled benign, so this is
    not a label-free quantity in general -- but it uses no detector scores and nothing at
    all from the evaluation window, which is what E6 is testing.  NOT gating on it lets the
    oracle pick a configuration the feasibility theorem forbids."""
    lut = {(r["pos"], r["family"], r["bucket_s"]): r for r in rows if r["seed"] == seed}
...
# ----------------------------------------------------------------------------------
# The [ORACLE] control: the same axis under e-LOND with a horizon-uniform gamma, which
# needs the horizon in advance and is therefore not deployable.  Reported so the transfer
# behaviour can be read against a procedure that already knows T.
# ----------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------
# [D6a] the feasibility gate: a function of |C|, k and T only, so it needs no tuning.
# What must still be reported is whether the feasible SET moves between windows.
# ----------------------------------------------------------------------------------
print("\n" + "=" * 118)
...
# ----------------------------------------------------------------------------------
# THE CAP AXIS.  t25_H5.json already covers all five positions and both seeds.
# ----------------------------------------------------------------------------------
...
res_cap = {}
# n_att_ep does NOT vary across caps within a window (verified: one value per (pos, seed)),
# so det_trunc/n_att_ep has a cap-independent denominator and D3a's objection does not apply
# to this axis.  Asserted rather than assumed.
...
for seed in SEEDS:
    # det_trunc is the number of attack episodes detected under the VALID (truncated) rule.
    D = cap_values(crows, "det_trunc", seed)
    NA = cap_values(crows, "n_att_ep", seed)
    V = D / NA
    # The alternative objective is the RAW rule, which is more powerful but is only a valid
    # e-value where group occupancy m <= n0 (section 4.12).  Comparing the two shows what
    # selecting on power alone would cost.
    ...
    # The cap does not change T, so the feasibility gate does not vary along this axis

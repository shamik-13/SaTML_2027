...
F9 records a measured benign firing rate 50.9x nominal at window position 0.85 (seed 0) and
24.3x (seed 1), against a median of 1.94x over the five positions.  At that position
E[e] = CEIL * P_fire ~ 51 rather than <= 1, so the constructed evidence is NOT a valid
e-value there and nothing computed at that position carries an FDR guarantee -- which
matters because 0.85 is the window every procedure comparison uses.

The firing event is exactly  s(x) > max_{z in C} s(z)  at k = 1, so the flows responsible
are identifiable.  This script extracts them and asks which of three explanations fits:

    distribution shift   -- deployment traffic is not exchangeable with calibration
    systematic label error -- the flows are attack traffic carrying a benign label
    new benign mode      -- a benign service/host that did not exist in calibration

and then applies the repair each explanation implies, measuring what it costs.

WHAT IS ORACLE AND WHAT IS NOT.  Part 6a (Mondrian calibration on a pre-committed field)
uses only information available before the deployment window opens.  Part 6b (excluding the
empirically dominant stratum) chooses the stratum by looking at the test-split tail and is
labelled [ORACLE] everywhere it appears; it answers "what would the number be if the cause
were removed", not "here is a deployable fix".

Outputs out/t30_A1.json.
"""
...
def _compromise_ips():
    ...
    External to the flow labels -- see t31_A2_alertaudit.py, which uses the same source."""
    import re as _re
    out = set()
...
# Label_src/Label_dst are a decomposition of Label, not an independent annotation.
# Establish that here rather than assuming it, because the audit script relies on it.
...
def _svc_key(service, dport):
    ...
    port is shifted by one rather than clipped -- clipping would alias missing with port 0."""
...
# host codes that ever appear on a labelled-malicious flow.  Two scopes, because they answer
# different questions: the whole-stream set is the forensic question ("is this host ever an
# attacker?"), the causal prefix is what a deployed system could have known at i2.
mal = y == 1
...
def _voidkeys(rows_a, rows_b):
    ...
    codes are comparable.  Returns (codes_a, codes_b)."""
    def _k(r):
        Z = np.ascontiguousarray(np.asarray(X[r], dtype=np.float32)) + 0.0   # kill -0.0
...
# ----------------------------------------------------------------------------------
# 1. Firing rates over all positions and seeds -- the table F9 quotes, recomputed here so
#    the deep dive at 0.85 is read against its own baseline.
# ----------------------------------------------------------------------------------
print("\n" + "=" * 112)
...
for pos in POS_ALL:
    ...
    for sd in SEEDS:
        ...
        # rank-depth profile on the same fit (no second detector fit)
...
# ----------------------------------------------------------------------------------
# 2. Is the excess only in the extreme tail, or is the whole benign score distribution
#    shifted?  Split conformal says P(rank <= k) = k/(|C|+1) for every k under
#    exchangeability, so the ratio as a function of k separates "a few outliers" from
#    "a global shift".
# ----------------------------------------------------------------------------------
print("\n" + "=" * 112)
...
# ----------------------------------------------------------------------------------
# 3. Composition of the extreme tail at position 0.85
# ----------------------------------------------------------------------------------
...
for sd in SEEDS:
    d = deep[sd]
    tail = np.flatnonzero((d["e_te"] > 0) & ben_w)          # benign-labelled, fired
    ...
    if len(tail) == 0:
        ...
        comp[sd] = c
        continue
    # the tail is small enough to enumerate, so it is written out in full -- a reviewer can
    # check the diagnosis against the raw records rather than against summary statistics
    ...
    # --- concentration by host and by categorical field
    ...
    # --- exact feature-vector collisions
    ...
    # --- novelty relative to the calibration window
    ...
    # --- how far above the ceiling are they
...
# ----------------------------------------------------------------------------------
# 3b. External corroboration.  The composition above says the tail is concentrated on a few
# hosts.  The red team's own task record says which hosts it compromised, and it is not
# derived from the flow labels, so it can adjudicate whether "benign extreme tail" means
# mislabelled attack traffic.
# ----------------------------------------------------------------------------------
print("\n" + "=" * 112)
...
for sd in SEEDS:
    ...
    # the firing rate among benign flows that the external record does NOT implicate
    clean = ben_w & ~touch_w
...
# How concentrated is the whole inflation?  Remove the host PAIRS that produced the tail and
# ask what is left.  [ORACLE]: the pairs are identified from the test-split tail itself, so
# this measures how localised the cause is -- it is not a deployable correction.
...
# ----------------------------------------------------------------------------------
# 4. Repairs.  (a) Mondrian conformal on a pre-committed field, NOT oracle.
#              (b) excluding the empirically dominant stratum, ORACLE.
# ----------------------------------------------------------------------------------
print("\n" + "=" * 112)
...
def run_stream(e_te_, y_te_, CEIL_, label):
    ...
    The horizon-uniform gamma = 1/T uses T, the number of episodes in the evaluation window.
    That is the project's standing horizon-knowledge ORACLE (section 7); it is used here so
    the rows are comparable with section 4.20, not because it is deployable.  ADDIS's poly
    sequence needs no horizon."""
...
def mondrian(s_cal_, y_cal_, s_te_, strat_cal, strat_te, k=1):
    ...
    flows of its own stratum, with its own ceiling (n_s+1)/k.  Valid within stratum by
    exchangeability within stratum; strata must be defined without using the score.
    A test flow whose stratum has no calibration flow cannot be calibrated -- it is given
    e = 0, which is conservative and is counted and reported."""
...
for sd in SEEDS:
    ...
    print(f"\n  --- seed {sd} " + "-" * 90)
    r = {}
    base_rate = float((d["e_te"][ben_w] > 0).mean())          # BENIGN flows only
    ...
    # (a) Mondrian on pre-committed fields -- available before deployment, NOT oracle
    for field in ("service", "conn"):
        st_cal = meta[field][slice(i1, i2)]
        st_te = meta_w[field]
        e_m, ceil_of, n_unc = mondrian(d["s_cal"], y[i1:i2], d["s_te"], st_cal, st_te, k=K)
        fr = float((e_m[ben_w] > 0).mean())                    # BENIGN flows only
        # E[e | benign] under the stratified rule uses each flow's own ceiling
        Ee = float(e_m[ben_w].mean())
        cmax = float(ceil_of.max())
        stream = run_stream(e_m, y_te, cmax, f"mondrian-{field}")
        # the feasibility ceiling is per-EPISODE under a stratified rule: the largest Ev an
        # episode can attain is the mean of its flows' own ceilings.  cmax (the global
        # maximum) is what the procedure run above uses, so its `silent` column is a LOWER
        # bound on silence; the per-episode margins below are the honest ones.
    ...
    # (b) ORACLE: drop the single (field, value) stratum holding the largest share of the
    #     tail, from BOTH calibration and deployment, and recalibrate.
    cand = []
    tail = np.flatnonzero((d["e_te"] > 0) & ben_w)
    if not len(tail):
    ...
    ben2 = y_te[keep_te] == 0
    fr2 = float((e2[ben2] > 0).mean())                         # BENIGN flows only

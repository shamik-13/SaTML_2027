...
Section 4.12 tested n0 = p99 and n0 = max only, and section 4.11 shows the detection floor
scales with n0, so the choice of cap carries a measurable detection cost that has never been
swept.  This sweeps n0 over mean, p50, p90, p99, p999 and max.

Caps are derived from CALIBRATION-window group sizes, never from the test window (standing
mistake 4).  Two rules are measured at each cap:

  raw        Sum(e)/n0 over the whole group.  This is a valid e-value ONLY where group
             occupancy m <= n0; where m > n0 it has E[.] = m/n0 > 1 and controls nothing.
             Reported so the cost of the invalid-but-powerful choice is visible.
  truncated  Sum over the FIRST n0 events of the group, divided by n0 (policy B of section
             4.12).  Valid at every cap by construction, and the front-load attack cost is
             what it trades for that.

Rejection threshold is thr = T/w0, the horizon-uniform level at zero prior rejections, as
in sections 4.11 and 4.12.

Detection floor: an episode fires when Sum(e)/n0 >= thr, and each firing flow contributes
CEIL, so at least ceil(thr*n0/CEIL) flows in the group must reach the evidence ceiling.
That is linear in n0 -- the floor scaling of section 4.11 -- and the measured floor below is
the smallest malicious-flow count among episodes actually detected.
"""
...
for pos in POS:
    ...
    for seed in SEEDS:
        ...
        nsz_g = np.bincount(gid, minlength=T)                       # group order, not episode order
        ...
        att = mal_g > 0
        # within-group rank in timestamp order (the window is already ts-sorted)
        ...
        for cname in CAPS:
            ...
            need = int(np.ceil(thr * n0 / CEIL))                    # firing flows required
            ...
            # front-load cost for the truncated rule: leading benign flows needed to push
            # all but (need-1) firing flows past the cap
            costs = []

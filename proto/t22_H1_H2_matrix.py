...
H2 -- rank k on real data.  Every real-data result so far fixes k = 1.

Both re-run the same quantities (sections 4.15, 4.17, 4.19), so they share one sweep:

    detector in {hgb, iforest} x position in {0.55..0.85} x seed in {0,1} x k in {1,10,100,1000}

The detector fit is the only expensive step and does not depend on k, so k is swept inside
each fit.  Episode grouping depends only on (position, bucket), so it is built once per
position.  Two-hour grouping throughout, matching sections 4.17 and 4.19.

DETECTOR NOTE (H1).  IsolationForest is unsupervised: it consumes no labels at fit time, so
its training rows are drawn from the whole training window rather than from the
label-balanced subsample the supervised detector uses.  This is a real difference in the
split design and it cuts both ways -- the unsupervised detector needs no attack labels at
all, but it also cannot exploit the ones LSPR23 provides.  Its hyperparameters (n_estimators=200,
max_samples=8192) were fixed in advance and never tuned against test performance.  They are
NOT sklearn's defaults -- sklearn uses n_estimators=100 and max_samples=min(256, n) -- they
are deliberately more generous on both axes, so the comparison does not handicap the
unsupervised detector.  contamination only sets the offset used by predict(); it does not
affect score_samples(), which is what is used here.  Conformal calibration still requires a BENIGN calibration
window, so labels are still needed there for both detectors -- being unsupervised removes
the label requirement from training only, not from calibration.

K NOTE (H2).  The threshold conformal e-value at rank k is e = ((|C|+1)/k) * 1{K <= k}, so
the ceiling falls as 1/k while the nominal benign firing rate rises as k/(|C|+1).  F11 says
the ceiling the procedures need is exactly where conditional validity is worst; the
measured/nominal firing ratio below is that trade-off on real scores rather than analytic.
The feasibility margin generalises to CEIL*w0/T - 1 with CEIL = (|C|+1)/k, which is the
section 4.13 corollary |C| >= kT/w0 - 1 rearranged.
"""
    ...
    for det in DETS:
        for seed in SEEDS:
            ...
            for k in KS:
                ...
                # flow-level vs episode-level at the best-performing procedure's alerts

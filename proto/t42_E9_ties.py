...
`h_stream.build_episodes` orders episodes with `np.lexsort((first_pos, first_ts))`
SPECIFICALLY so that equal-timestamp episodes are ordered deterministically (standing
mistake 5).  This quantifies the variance that choice suppresses; it is not fixing a bug.

Every design rule is derived and numerically verified in t42a_E9_derivation.py.  A
disagreement between the two is a bug, not a finding.  The derivation shaped this script in
three ways, marked [D..] where they bite:

  D2a  the EXPOSURE -- how many episodes are even eligible to move -- needs no detector and
       no labels, so it is computed first at every position, and decides how much is spent
       on seeds.
  D3a  a tie block propagates past itself only by changing its own rejection count, so the
       within-block and beyond-block effects are reported separately.
  D4a  smallest-p-first is NOT the block optimum (refuted by brute force), so there is no
       cheap surrogate: extremes come from the seeds and are labelled as SAMPLES.

Randomisation replaces the SECONDARY sort key only, through build_episodes' `tie_key`
argument, so the across-timestamp order is untouched by construction rather than by care.

Outputs out/t42_E9.json.
"""
...
POS_EXPOSURE = [0.55, 0.62, 0.70, 0.77, 0.85]     # exposure is free: no detector needed
POS_FULL = [0.55, 0.85]                            # guarantee and stress windows
K = 1; A = 0.05; W0 = 0.025; BUCKET = 2 * 3600; SEED = 0
NSEED = 50                                         # the workplan's 50 randomised orders
LAM, TAU = 0.25, 0.5
...
# =======================================================================================
# Module level so t42b_E9_selftest.py can import and mutate the SHIPPED code.
# =======================================================================================
def tie_exposure(first_ts):
    """The movable population, from timestamps alone.  [D2a]

    Returns the episode count, the number of distinct first_ts blocks, the number of
    episodes sitting in a block of size >= 2 (an EXACT upper bound on how many episodes any
    tie-break can move), the number of such blocks, and the largest block size.  No
    detector, no labels, no procedure.
    """
...
def first_ts_by_group(ts_w, gid, T):
    ...
    order, before the lexsort).  Same computation build_episodes performs."""
...
def stats(fired, ismal, n_mal):
    ...
    FDP is None with no rejections, not 0.0 -- a collapsed run must not read as "no false
    discoveries" (the same convention E4 uses).
    """
...
def percentile_of(value, sample):
    ...
    fraction of randomised draws strictly below it plus half the ties -- the mid-rank
    convention, so a constant statistic reports 0.5 rather than 0.0 or 1.0.       [D6]"""
...
# ----------------------------------------------------------------------------------
# [D2a] EXPOSURE FIRST.  Detector-free and label-free, so it costs nothing and it decides
...
for pos in POS_FULL:
    ...
    for sd in range(NSEED):
        ...
        # POWER CHECK.  Every statistic below coming out with sd = 0 is only a NULL RESULT
        # if the randomisation actually moved the stream.  If tie_key happened to reproduce
        # the deterministic order, sd = 0 would be an artefact of a seed that perturbs
        # nothing.  Count how many episode positions each draw actually changes.
        nmoved = int((rep["order"] != det_ep["order"]).sum())
        n_moved_total += nmoved
        n_order_changed += int(nmoved > 0)
        rng_seed_orders.append(nmoved)
        # [D1a] the randomisation must not touch the across-timestamp order
        if sd == 0:
    ...
    for nm in ("lond", "lordpp", "addis"):
        for stat_name in ("rejections", "tp", "fdp", "recall", "first_rank",
            ...
            # "does it vary" must be an EXACT test.  np.std(ddof=1) over 50 bitwise
            # identical float64 values returns ~1e-17, not 0, so `sd > 0` reports float
            # noise as variation -- it flagged 4 statistics at position 0.55 whose range
            # was a single value.  Count distinct values instead.
            key = lambda v: None if v is None else float(v)
            support = {key(v) for v in vals}
            n_distinct = len(support)
            # The deterministic value has to enter the SAME support set.  Comparing it only
            # against the finite draws misses the two mixed cases: deterministic numeric
            # with every draw None, and deterministic None with every draw numeric.
            ...
            row = dict(pos=pos, proc=nm, stat=stat_name,
                       ...
                       # percentile_of drops None draws, so it is only interpretable when
                       # the statistic is defined in every draw; the fraction is reported
                       # so an "extreme 5% tail" reading cannot be taken on a statistic

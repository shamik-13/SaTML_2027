...
Originally scoped as "sweep q in {0.01, 0.05, 0.10, 0.20}".  Section 4.20 showed the
spending sequence gamma moves the realised operating point by a factor of 2.0 in recall,
which is larger than anything q does, so sweeping q alone would attribute to q an effect
that belongs to gamma.  This is therefore a JOINT sweep over (q, gamma).

Throughout, w0 = q/2, matching the convention used everywhere else in the record.

The section 4.13 corollary predicts feasibility over horizon T requires |C| >= kT/w0 - 1,
i.e. the margin (|C|+1)*w0/(kT) - 1 is LINEAR IN w0 and hence in q.  The check below is
whether (margin + 1) really scales in proportion to q at fixed stream: quadrupling q from
0.05 to 0.20 should exactly quadruple (margin + 1).

5 positions x 2 seeds, k = 1, two-hour grouping.
"""
...
for pos in POS:
    ...
    for seed in SEEDS:
        ...
        for q in QS:
            ...
            # CEIL is already (|C|+1)/k, so dividing by k again double-counts it:
            # |C| >= kT/w0 - 1  <=>  (|C|+1)*w0/(kT) >= 1  <=>  CEIL*w0/T >= 1
...
# ---- 1. does the margin scale linearly in q, as the corollary predicts? -------------
print("\n" + "=" * 112)
...
# ---- 2. what q actually buys, per gamma --------------------------------------------
for gk in GAMMAS:
    print("\n" + "=" * 112)
...
# ---- 3. q against gamma, head to head ----------------------------------------------
print("\n" + "=" * 112)

...
Design: FIXED-SIZE sliding windows. train | cal | test blocks of constant size are slid to
5 positions across the stream, so T, |C| and the feasibility margin vary because the TRAFFIC
differs, not because the window sizes differ. Two detector seeds per position vary the
scores (hence detections and dilution cost) while leaving T and |C| untouched.
Splits are defined on the full timestamp-sorted 16,353,511-flow stream before any
subsampling; benign subsampling is confined to the training window.
"""
...
src=df["src"].cat.codes.to_numpy(dtype=np.int32,copy=True)      # integer codes, not strings
...
W0=0.025; K=1
CAL_F, TEST_F = 0.15, 0.15                       # fixed window sizes
POS=[0.55,0.62,0.70,0.77,0.85]                   # test-window start, fraction of N
SEEDS=[0,1]
results=[]
for pos in POS:
    i2=int(pos*N); i1=i2-int(CAL_F*N); i3=min(N,i2+int(TEST_F*N))
    if i1<=0: continue
    # grouping depends only on (i2,i3,BH) -- compute once per position, reuse across seeds
    groups={}
    for BH in (1,2,6):
    ...
    for seed in SEEDS:
        ...
        for BH,(gid,T,nsz,mal) in groups.items():
            ...
            thr=T/W0; margin=CEIL/thr-1.0                 # feasible iff margin >= 0

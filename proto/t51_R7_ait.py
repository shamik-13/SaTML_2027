...
AIT_DIR = os.environ.get("AIT_DIR", "/tmp/ait_cache")   # tstat netflows extracted here from proto/data


def ensure_extracted():
    """Extract each scenario's tcp_complete.csv from src/data/ait/NF__<name>_netflows.zip if absent."""
    data = Path(__file__).resolve().parents[1] / "src" / "data" / "ait"   # moved: src/ is canonical
...
def load_scenario(path):
    ...
    return dict(y=y, ts=ts, Xnum=Xnum.fillna(0.0), good=good, H=H,
                dst_ip=df["s_ip"].to_numpy(),

...
Design (mirrors t48 at the primary guarantee window 0.55 and the stress window 0.85, seed 0):
  ...
  Part C -- the padding-transfer boundary, with a CAUSAL ACCUMULATION REPLAY (not a static graft).
    ...
    The black-box pad pool is the window's most common (proto,dport) among flows the deployed detector
    does NOT fire on (chosen from detector outputs, NOT y labels); its benign fraction is audited.
...
def main():
    ...
    for pos in POS:
        ...
        # BLACK-BOX pad pool: most common (proto,dport) among flows the DETECTOR does not fire on ----
        # (chosen from detector outputs, no ground-truth labels), and its measured fail rate.
        nonfire = eH_te == 0
        svc = pr_w.astype(np.int64) * 100000 + dp_w.astype(np.int64)
        uu, cc = np.unique(svc[nonfire], return_counts=True); bb_svc = uu[np.argmax(cc)]
        pool_idx = np.flatnonzero(nonfire & (svc == bb_svc))
        ...
        partC[f"{pos}"] = dict(
            ...
            ablation_mean_fire_drop=mean_drop, responsible_feature=responsible, per_episode=recs)
    ...
    out = dict(
        config=dict(POS=POS, SEED=SEED, bucket_s=BUCKET, k=K, w0=W0, alpha=A,
                    ...
                    pool="black-box: most common (proto,dport) among detector-non-firing flows (no y labels)",
        ...
        note=("Host-conditioned HGB on X_aug=[33 flow stats | 6 strict-ts-causal host features]; no "
              ...
              "benign calibration score. Conclusion rests on the fire rate (no suppression cost r* is "
              "computed: HGB is non-monotone in these features and huge r may not fit the bucket). Pool "
              "is black-box (detector-non-firing flows, no labels; benign fraction audited). At the "
              "primary window ordinary pads fire 0 over 4e6 trials and the detector fires on 0 real "
              "benign flows (holds). At the stress window grafted attack-context pads fire but the "
              "detector fires on ~0 real benign flows -> out-of-distribution extrapolation (attacked "
              "pairs are 100% malicious), so the high-volume boundary needs a testbed. Windows 0.55, 0.85."))

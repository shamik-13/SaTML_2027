...
UNIT RECOVERY that has to happen before any of it is meaningful.

Runs before t41_E8_units.py touches LSPR23.  Same contract as t34a/t35a/t36a/t38a/t39a/t42a.

No LSPR23 flow data is read here.  Runtime ~5 s.

======================================================================================
WHY A "PURE ARITHMETIC" TASK NEEDS A DERIVATION
======================================================================================
docs/02_WORKPLAN_PHASE4.md E8 calls this "pure arithmetic over cached columns".  The
arithmetic is trivial; the UNITS are not, and every number E8 produces is a unit conversion
away from being wrong by a factor of 10^3 or 10^6.  Specifically:

  * `Flow Duration` (LSPR23 column 9, cached as f0) has no documented unit in the record.
    CICFlowMeter-derived files use microseconds, but that is an assumption, not a fact
    about this file.  A required-bandwidth figure inherits that factor directly.
  * `Total Length of Fwd/Bwd Packet` are PAYLOAD lengths.  Link-level and header bytes are
    in columns 42-43, which the cache does NOT carry (_FEAT stops at column 40).  A "bytes
    on the wire" figure that quotes payload only UNDERSTATES the attack's cost, which is
    the direction that flatters the attacker.
  * the attack budgets are counts of flows drawn from a POOL, and a pool has a
    distribution, not a mean.  An attacker picks; the mean is the wrong summary.

D1 recovers the duration unit from internal consistency rather than assuming it.  D2 fixes
the estimator.  D3 states what the byte figure does and does not include.

======================================================================================
COLUMN MAP (docs/01_HANDOFF_PHASE4.md section 2.2: /tmp/lspr_full.csv is ts, src, dst,
label, proto, f0..f31, where f_i is ls23pr_v1.csv column i+9, and h_stream's X is
[proto] + f0..f31, so X[:, j] is column j+8 for j >= 1)
======================================================================================
    X[:, 0]  Protocol            (col 6)
    X[:, 1]  Flow Duration       (col 9)     unit RECOVERED in D1, not assumed
    X[:, 2]  Flow Bytes/s        (col 10)
    X[:, 3]  Flow Packets/s      (col 11)
    X[:, 4]  Tot Fwd Pkts        (col 12)
    X[:, 5]  Tot Bwd Pkts        (col 13)
    X[:, 6]  Total Length of Fwd Packet  (col 14)
    X[:, 7]  Total Length of Bwd Packet  (col 15)
======================================================================================
"""
...
# =======================================================================================
# The primitives.  Module level so t41 imports the SAME code, not a copy.
# =======================================================================================
...
def recover_duration_scale(duration, total_bytes, bytes_per_s, min_dur=0.0,
    ...
    The file carries both the totals and the rates, so for the correct scale S

        bytes_per_s  ==  total_bytes / (duration / S)

    holds on every flow with a positive duration.  Trying S in {1, 1e3, 1e6, 1e9} and
    picking the one that minimises the median absolute log-ratio identifies the unit from
    the data rather than from a convention.  Returns (best_scale_name, table), where the
    table gives the median |log10 ratio| for each candidate -- a spread of several orders
    of magnitude between candidates is what makes the identification safe, and the caller
    ASSERTS that spread rather than trusting the argmin.

    Also returns the SURVIVOR FRACTION.  A confident-looking table computed on a tiny
    subset of flows is not evidence about the file's unit, so the fraction is reported and
    the caller asserts it is large.  The default filters are strictly positive values only,
    not an arbitrary magnitude cut.
    """
...
def per_flow_cost(X, rows, header_bytes_per_packet=0.0):
    ...
    `Total Length of Fwd/Bwd Packet` are PAYLOAD bytes; the header columns are not in the
    cache.  header_bytes_per_packet adds a stated per-packet allowance so the wire figure
    can be reported as a range rather than as a payload-only number that understates the
    attacker's cost.  40 bytes is the TCP/IPv4 minimum.
    """
...
def budget_cost(pkts, bytes_, n_flows, quantiles=(0.0, 0.5, 1.0)):
    ...
    "n times a quantile" is a proxy, not a model, and different models give materially
    different answers.  Three are computed and named, so the headline cannot quietly be a
    fourth thing:

      iid_expected   n * mean(pool)          the attacker samples the pool at random
      typical        n * median(pool)        the attacker sends a typical flow n times
      chosen_min     n * min(pool)           the attacker repeats the single cheapest flow
      chosen_sum     sum of the n cheapest   the attacker sends n DISTINCT cheapest flows;
                                             None when n exceeds the pool, where sending n
                                             distinct pool flows is not possible at all

    The quantile proxies are kept for the spread, labelled as proxies.
    """
    out = {}
    for q in quantiles:
...
for true_name, S in SCALES.items():
    n = 20000
    dur_s = rng.uniform(1e-4, 30.0, n)                       # true duration in SECONDS
    by = rng.uniform(60, 1e6, n)
    bps = by / dur_s
    dur_col = dur_s * S                                      # as the file would store it
    ...
    check_bool(f"D1a  recovers '{true_name}' from synthetic columns", got == true_name,
               note=f"runner-up {ranked[1][0]} at {ranked[1][1]:.2f} decades vs "
                    f"{ranked[0][1]:.2e}")
# and the separation is large, which is what makes the assertion in t41 safe
...
# with noise in the rate column the recovery must still work
...
""")
r = rate_and_hosts(7200.0 * 1e6 / 8.0, 7200.0)     # exactly 1 Mbit/s for 2 hours
...
r2 = rate_and_hosts(7200.0 * 1e9 / 8.0, 7200.0)    # 1 Gbit/s

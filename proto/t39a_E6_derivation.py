...
Runs before t39_E6_transfer.py touches LSPR23.  Same contract as t34a/t35a/t36a/t38a: every
closed form and every design rule the real-data script relies on is derived here and checked,
so a disagreement between the two is a bug rather than a finding
(docs/01_HANDOFF_PHASE4.md section 6).

No data files are read (t39_E6_transfer.py is parsed for its two selection functions,
not executed).  Runtime ~15 s.

======================================================================================
THE QUESTION
======================================================================================
docs/02_WORKPLAN_PHASE4.md E6: "Were the grouping width, cap, spending sequence or
threshold chosen after looking at the test window?"  Accept when the main qualitative
findings are shown not to require test-window tuning, or the ones that do are named.

Three things have to be settled BEFORE any number is produced, because each of them can
turn a null result into a false positive or the reverse:

  D2  a transfer experiment on a FLAT grid is vacuous -- every rule ties, regret is zero,
      and "no tuning needed" is reported when the truth is "nothing was tunable".  The
      spread has to be reported next to the regret, and the regret normalised by it.
  D3  RECALL IS NOT COMPARABLE ACROSS GROUPING FAMILIES, because the family sets the
      denominator.  Selecting a grouping by episode recall selects the coarsest grouping
      almost by construction.  The objective must be flow coverage.
  D6  the feasibility gate is a function of |C|, k and T only (F5) -- no scores, and
      nothing from the evaluation window -- so it transfers EXACTLY and must not be counted
      as a tuned parameter.  (|C| does use calibration-window labels; that is not the same
      thing as using the window it is evaluated on.)

======================================================================================
NOTATION
======================================================================================
    G            the configuration grid (grouping family x bucket width x spending family)
    W            the ordered deployment windows (positions 0.55, 0.62, 0.70, 0.77, 0.85)
    V_j(c)       the objective of config c measured on window j
    oracle_j     max_c V_j(c)                        uses window j's labels: not attainable
    prev_j       V_j(argmax_c V_{j-1}(c))            E6a, previous-window selection
    loo_j        V_j(argmax_c mean_{i != j} V_i(c))  E6b, leave-one-window-out
    frozen_j     V_j(c*), c* the record's headline (src-dst, 7200 s, gamma prop j^-1.6)
======================================================================================
"""
...
# =======================================================================================
# The selection primitives are NOT redefined here.  t39_E6_transfer.py owns them, and this
# file pulls them OUT of that source by AST and executes them, so the derivation verifies
# the SHIPPED implementation rather than a second copy of it that can drift from it
# (docs/01_HANDOFF_PHASE4.md section 8 step 7).  t39 reads JSON at module level, so it
...
# random selection on a non-flat grid has expected rho = (mean-adjusted) 1 - (mean-min)/(max-min)
G = 200
vv = rng.random(G)
...
""")
# the concrete construction: coarse wins on recall while losing on coverage
mal_flows = np.array([100, 1, 1, 1, 1])        # one big attack episode and four small ones
fine_fired = np.array([False, True, True, True, True])
fine_recall = fine_fired.sum() / 5
fine_cov = mal_flows[fine_fired].sum() / mal_flows.sum()
coarse_recall = 1.0                            # one merged episode, and it fires
coarse_cov = 1.0
...
# and the reverse direction: coarse grouping can lose coverage while keeping recall at 1
mal2 = np.array([100, 1, 1, 1, 1])
coarse2_fired = np.array([False, True, True, True, True])   # the big one merged away, say
check_bool("D3b  a family can raise recall while lowering coverage",
           (coarse2_fired.sum() / 5) > (mal2[coarse2_fired].sum() / mal2.sum()))
# the degenerate limit, stated as a check rather than as prose
...
""")
# sqrt(2 ln G) is asymptotic and only sizes the effect.  Checking the simulation against
# it to within 30% has no power -- a formula missing a factor of sqrt(2) passes.  The EXACT
# value E[max] = integral x d(Phi(x)^G) is computed by quadrature and the simulation is
# checked against THAT, which does have power; the asymptotic is then reported beside it
# as the crude sizing rule it is.
...
for G in (10, 70, 500):
    ...
    se = 5.0 / math.sqrt(n)          # 5 sigma, sd of the max is < 1 for these G
...
# An all-NaN objective and an infinite one are REFUSED rather than silently resolved: a
# fallback to the first declared config would report a tie-break as a selection, and
# select() on a grid containing +inf would return a finite value that is not the maximum.
def _raises(fn):
    try:
        fn(); return False
    except ValueError:
...
# The feasible set moves with T alone, and the crossover is exact:
#     margin >= 0  <=>  T <= (|C|+1)*w0/k                                            [D6b]
NC = 1_813_113
Tstar = (NC + 1.0) * W0 / 1.0

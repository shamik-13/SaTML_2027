...
Two of the configurations in t21_H6_procedures.py are feasible over the whole stream only
because they are told the exact stream length T in advance:

  * horizon-uniform gamma_j = 1/T   (max-min optimal, section 4.5)
  * e-GAI with w_1 = 1/T           (the value arXiv 2506.01452 itself recommends)

T is the number of episodes in the evaluation window, so setting either from T is oracle
knowledge.  A deployed operator commits to a guess T_hat instead.  This script sweeps
T_hat = c * T and reports how fast each procedure degrades, which is what says whether the
escape is a property of the procedure or of the tuning.

ADDIS under gamma prop j^-1.6 is the control: it needs no horizon at all, because its
gamma index is driven by discarding rather than by elapsed time.

Position 0.85, two-hour grouping, seed 0 -- identical stream to t21_H6_procedures.py.
"""
...
def gamma_uniform_hat(That, T):
    ...
    its entire budget before the stream ends and is silent thereafter by construction."""

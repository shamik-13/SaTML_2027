...
detector seeds of section 4.15, so that no number from t21_H6_procedures.py enters the
record as a single-draw result (standing rule 5).

Window design is identical to t17_T2_fullstream.py: fixed-size train | cal | test blocks
slid to five positions, two detector seeds per position, benign subsampling confined to
the training window.  Grouping is two-hour, matching sections 4.17 and 4.19.

Reports, per procedure and per spending sequence, the distribution over the ten
(position, seed) configurations of: rejections, realised FDP, episode recall, silence rate.
The FDP column is the one that matters: FDR control is a statement about the mean of FDP,
so a procedure that appears to escape the section 4.13 feasibility template is only
interesting if its FDP stays at or below q = 0.05 across configurations.
"""
